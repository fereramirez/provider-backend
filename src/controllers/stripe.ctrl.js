    ...
    try {
        ...
        for (const prod of order.products) {
            items.push({
                price_data: {
                    currency: "ars",
                    unit_amount: prod.on_sale
                        ? stripePrice(prod.sale_price)
                        : stripePrice(prod.price),
                    product_data: {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            shipping_options: [
                {
                    shipping_rate_data: {
                        display_name: order.flash_shipping
                            ? `Flash${order.free_shipping ? " & free shipping applied" : " shipping"
                            }`
                            : order.free_shipping
                                ? "Free shipping"
                                : "Standard shipping",
