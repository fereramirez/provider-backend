...
const getUserNotifications = async (req, res, next) => {
  try {
    const firstNotif = {
      ...
      description:
        "Gracias por crear una cuenta en nuestra tienda. Esperamos que disfrutes de lo que tenemos para ofrecerte. No olvides pasar por la sección de preguntas frecuentes para quitarte cualquier duda que puedas tener.",
      link: "/faqs",
    ...
    if (!userId)
      return res.json({ error: true, message: "ID de usuario no necesaria" });
    ...
    if (!userNotifs) {
      ...
      const response = await Notifications.findByIdAndUpdate(
        newNotif.id,
        {
          $push: {
...
const deleteNotification = async (req, res, next) => {
  try {
    ...
    if (!userId)
      return res.json({ error: true, message: "ID de usuario necesaria" });
    if (!target)
      return res.json({ error: true, message: "ID de notificación necesaria" });
    ...
    if (updatedNotfis)
      return res.json({
        ...
        notif_list: updatedNotfis.notif_list,
      });
...
const post = async (req, res, next) => {
  try {
    ...
    const { notif_type, title, description, link } = req.body;
